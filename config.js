...
function saveAppState(appState) {
    ...
    console.log("Da luu appState de lan sau dung lai");
}
...
⚠️ CAN VPN HAN QUOC!`;
    ...
    if (cmd === "info") {
        const uniq = args[1];
        const host = args[2];  // Bắt buộc phải nhập, không còn mặc định
        const sv = args[3] || "1";
        
        if (!uniq) return api.sendMessage("📝 !info <uniq_id> <host_id> [server]", threadID);
        if (!host) return api.sendMessage("📝 !info <uniq_id> <host_id> [server]\n\n🔴 Vui long nhap HOST_ID!", threadID);
...
async function startBot() {
    ...
    if (!FB_EMAIL || !FB_PASSWORD || FB_PASSWORD === "mat_khau_cua_ban") {
        ...
        console.log("📌 Sua file index.js, tim dong:");
        console.log("   const FB_EMAIL = '...'");
        ...
        login({ email: FB_EMAIL, password: FB_PASSWORD }, (err, _api) => {
            if (err) {
                ...
                if (err.error === 'login-approval') {
                    console.log("\n⚠️ TAI KHOAN CAN XAC MINH 2FA!");
                    console.log("📌 Vao Facebook de xac minh, hoac dung appState!");
                }
                return;
            }
            ...
            // Luu appState de lan sau dung lai
            const appState = api.getAppState();
            fs.writeFileSync(appStatePath, JSON.stringify(appState, null, 2));
            console.log("💾 Da luu appState de lan sau!");
